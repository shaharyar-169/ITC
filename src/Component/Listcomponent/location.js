...
export default function LocationList() {
    ...
    const dashboardUrl = "https://crystalsolutions.com.pk/iqbaltrader/web/Location.php";
    ...
    const handlePdfDownload = () => {
        ...
        const headers = ['Code', 'Description', 'Name','Sts'];
...
const getTotalTableWidth = () => {
    ...
    const handleDownloadCSV = () => {
        ...
        const headers = ['Code', 'Company Description', 'Name', 'Sts'];
    ...
    useEffect(() => {
        ...
        const filteredData = filteredDailysale.filter(item => {
            ...
            if (searchQuery !== '') {
                return item.TLocDsc && item.TLocDsc.toLowerCase().includes(searchQuery.toLowerCase());
            }
            return true;
        })
        ...
        if (filteredData.length === 0) {
            ...
            const prevFilteredByQuery = previousFilteredData.filter((item) => {
                const itemDesc = item.TLocDsc ? item.TLocDsc.toLowerCase() : '';
        ...
        if (selectedTransactionType !== 'All') {
            filteredData = filteredData.filter((item) => {
                if (selectedTransactionType === 'Active') {
                    return item.TLocSts === 'Y';
                } else if (selectedTransactionType === 'Non-Active') {
                    return item.TLocSts === 'N';
                }
                return true; // For 'All', return all data
            });
        }
    ...
    const handleCodeHeaderClick = () => {
        ...
        const sortedData = [...filteredDailysale].sort((a, b) => {
            ...
            return order * (a.TLocCod.localeCompare(b.TLocCod));
        });
    ...
    //     // Sort the data based on the sorting order of TLocCod
    //     const sortedData = [...filteredDailysale].sort((a, b) => {
    //         const order = isCodeSortedAscending ? 1 : -1;
    //         return order * (a.indices[index].localeCompare(b.indices[index]));
    //     });
    
    //     // Update the state with the sorted data and update index values
    //     const sortedDataWithIndex = sortedData.map((item, index) => ({ ...item, index: index + 1 }));
    //     setFilteredDailysale(sortedDataWithIndex);
    // };
...
filteredDailysale.forEach((item, index) => {
    // Push an object with id field into the indices array
    indices.push({ id: (index + 1).toString() });
});

// Access each object in the indices array and log its id property
indices.forEach((obj) => {
    console.log("index id: " + obj.id);
});
    ...
    return (
        <div>
            ...
            <Container style={{  width: '520px', display: 'flex', justifyContent: 'center', border: '1px solid #cccccc' }} className="mt-3 ">
                <div className="pt-2 pb-2">
                <div className="parent ">
                        <div className="child1 mb-1 justify-content-between" >
                            ...
                            <div style={{ marginLeft: '10px' }}>
                                <label htmlFor="searchInput" style={{ paddingRight: '10px' }}><span  style={{ fontSize: '12px', fontWeight:"bold" }}>Search:</span> </label>
                                <input
                                    type="text"
                                    id="searchInput"
                                ...
                                {showSpinner ? (
                                    ...
                                    filteredDailysale
                                        ...
                                        >
                                                ...
                                                <td style={{ width: '20px'}}>{item.TLocCod}</td>
                                                <td className='table-col-item-description' style={{ width: '200px' }}>{item.TLocDsc}</td>
                                                <td className='table-col-item-description' style={{ width: '200px' }}>{item.TLocNam}</td>
                                                <td style={{ width: '20px' }}>{item.TLocSts}</td>
