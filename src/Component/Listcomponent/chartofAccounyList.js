...
export default function ChartOfAccountList() {
    ...
    const [isCodeSortedAscending, setIsCodeSortedAscending] = useState(false);
    const [issortforstatus, setissortforstatus ] =useState(false);
    const [issortforDescription , setissortforDescription] =useState(false);

//     const [isCodeSortedAscending, setIsCodeSortedAscending] = useState(false);
// const [issortforDescription, setissortforDescription] = useState(false);
// const [isStatusSortedAscending, setIsStatusSortedAscending] = useState(false);
    ...
    const dashboardUrl = "https://crystalsolutions.com.pk/iqbaltrader/web/ChartOfAccount.php";
...
const getTotalTableWidth = () => {
    ...
    // const handlePagination = () => {
    //     // Define the addTitle function
    //     const addTitle = (title, date, time, pageNumber, startY, titleFontSize = 16, dateTimeFontSize = 8, pageNumberFontSize = 10) => {
    //         doc.setFontSize(titleFontSize); // Set the font size for the title
    //         doc.text(title, doc.internal.pageSize.width / 2, startY, { align: 'center' });
    ...
    //         if (date || time) {
    //             let dateTimeString = '';
    //             if (date) dateTimeString += date;
    //             if (time) {
    //                 if (dateTimeString) dateTimeString += ' ';
    //                 dateTimeString += time;
    //             }
    //             doc.setFontSize(dateTimeFontSize); // Set the font size for the date and time
    //             doc.text(dateTimeString, rightX, startY, { align: 'right' });
    //         }
        
    //         // Add page numbering
    //         doc.setFontSize(pageNumberFontSize);
    //         doc.text(`Page ${pageNumber}`, rightX - 30, startY, { align: 'right' });
    //     };
    ...
    //         const date = getCurrentDate(); // Get current date
    //         const time = getCurrentTime(); // Get current time
    //         addTitle('IQbal Trading Company', date, '', '', startY); // Render company title with only page number
    //         startY += 5; // Adjust vertical position for the company title
    //         addTitle('Chart of A/C Report', '', time, '', startY, 10, 8); // Render sale report title with decreased font size, provide the date, time, and page number
    //         startY += 10; // Adjust vertical position for the sale report title
    //         addTitle('', '', '', pageNumber, startY, 10, 8); // Render sale report title with decreased font size, provide the date, time, and page number

          
    //         addTableHeaders((doc.internal.pageSize.width - totalWidth) / 2, startY, date, time); // Pass date and time to the table headers function
    //         const startIndex = currentPageIndex * rowsPerPage;
    //         const endIndex = Math.min(startIndex + rowsPerPage, rows.length);
    //         startY = addTableRows((doc.internal.pageSize.width - totalWidth) / 2, startY, startIndex, endIndex);
    //         if (endIndex < rows.length) {
    ...
    useEffect(() => {
        ...
        const filteredData = filteredDailysale.filter(item => {
            ...
            if (searchQuery !== '') {
                return item.taccdsc && item.taccdsc.toLowerCase().includes(searchQuery.toLowerCase());
            }
            return true;
        })
        ...
        if (filteredData.length === 0) {
            ...
            const prevFilteredByQuery = previousFilteredData.filter((item) => {
                const itemDesc = item.taccdsc ? item.taccdsc.toLowerCase() : '';
        ...
        if (selectedTransactionType !== 'All') {
            filteredData = filteredData.filter((item) => {
                if (selectedTransactionType === 'Active') {
                    return item.taccsts === 'A';
                } else if (selectedTransactionType === 'Non-Active') {
                    return item.taccsts === 'N';
                }
                return true; // For 'All', return all data
            });
        }
...
const handleCodeHeaderClick = () => {
        // Toggle the sorting order when the header is clicked
        setIsCodeSortedAscending((prev) => !prev);
        // Sort the data based on the sorting order
        const sortedData = [...filteredDailysale].sort((a, b) => {
            const order = isCodeSortedAscending ? 1 : -1;
            return order * (a.tacccod.localeCompare(b.tacccod));
        });
...
const handlestatusSortingClick = () => {
    // Toggle the sorting order when the header is clicked
    setissortforstatus((prev) => !prev);

    // Sort the data based on the sorting order
    const sortedData = [...filteredDailysale].sort((a, b) => {
        const order = issortforstatus ? 1 : -1;
        return order * a.taccsts.localeCompare(b.taccsts, undefined, { numeric: false });
    });
...
const handleDescriptionSortingClick = () => {
    // Toggle the sorting order when the header is clicked
    setissortforDescription((prev) => !prev);

    // Sort the data based on the sorting order
    const sortedData = [...filteredDailysale].sort((a, b) => {
        const order = issortforDescription ? 1 : -1;
        return order * a.taccdsc.localeCompare(b.taccdsc, undefined, { numeric: false });
    });
    ...
    return (
        <div>
            ...
            <Container style={{ width: '600px', display: 'flex', justifyContent: 'center', border:'1px solid #cccccc' }} className="mt-2 ">
                <div className="pt-2 pb-2 " style={{width:'500px'}}>
                <div className="parent ">
                        <div className="child1 justify-content-between mb-1" >
                        ...
                        <MDBTable responsive striped scrollY maxHeight="570px"id='dailysale-table' className="MDB-table" >
                            <MDBTableHead style={{ position: 'sticky', top: '0', backgroundColor: '#fff' }}>
                                ...
                                <th
                                        ...
                                        onClick={handleCodeHeaderClick}
                                    >
                                        Code
                                        <span
                                            style={{
                                                display: 'inline-block',
                                                transform: isCodeSortedAscending ? 'rotate(180deg)' : 'none',
                                    ...
                                    <th
                                        ...
                                        onClick={handleDescriptionSortingClick}
                                    >
                                        Description
                                        <span
                                            style={{
                                    ...
                                    <th
                                        ...
                                        onClick={handlestatusSortingClick}
                                    >
                                        Sts
                                        <span
                                            style={{
                            ...
                            <MDBTableBody id="dailysale-table-body">
                                ...
                                {showSpinner ? (
                                    ...
                                    filteredDailysale
                                        .map((item, index) => (
                                            <tr key={index}>
                                                <td style={{ width: '70px' }}>{item.tacccod}</td>
                                                <td className='table-col-item-description' style={{ width: '300px' }}>{item.taccdsc}</td>
                                                <td style={{ width: '20px'}}>{item.taccsts}</td>
                                ...
                                {Array.from({ length: Math.max(24 - filteredDailysale.length, 0) }).map((_, rowIndex) => (
                                    <tr key={`empty-row-${rowIndex}`}>
                                        ...
                                        <td className={` ${filteredDailysale.length === 0 ? 'empty-style' : ''}`} style={{ width: '20px' }}></td>

                                    </tr>
                                ))}
                              
                                {/* {Array.from({ length: Math.max(0, 20 - filteredDailysale.length) }).map((_, rowIndex) => (
                            ...
                            <tr style={{ position: 'sticky', bottom: '0', backgroundColor: 'rgb(235, 135, 64)' }}>
                                <th className="text-left" colSpan={3}>
                                    <FontAwesomeIcon icon={faFilePdf} style={{ marginLeft:'10px'}} className="custom-icon" onClick={handlePdfDownload} title="Download PDF" />
                                    <FontAwesomeIcon icon={faFileCsv} style={{ marginLeft:'10px'}} className="custom-icon" onClick={handleDownloadCSV} title="Download CSV" />
                                </th>
