...
const Get_Collector = () => {
  ...
  const apiLinks =
    "https://crystalsolutions.com.pk/iqbaltrader/web/CollectorList.php";
  const imageurl = `https://www.crystalsolutions.com.pk/csart/itemimage/`;
  const [Length, setLength] = useState("");

  const handleMenuItemClick = () => {
    navigate("/Add_Collector");
  };
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const transformedData = data.map((item) => ({
          id: item.id,
          TColNam: item.TColNam,
          TColSts: item.TColSts,
        }));
  ...
  const filteredRows = data.rows.filter(
    (row) =>
      (row.TColNam &&
        row.TColNam.toLowerCase().includes(searchText.toLowerCase())) ||
      (row.TColSts &&
        row.TColSts.toLowerCase().includes(searchText.toLowerCase()))
  );
