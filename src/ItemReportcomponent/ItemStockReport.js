...
export default function ItemStockReport() {
    ...
    const dashboardUrl = "https://crystalsolutions.com.pk/iqbaltrader/web/ItemStockReport.php";
    ...
    const totalAmount = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.stock.replace(/,/g, '') || 0), 0);
    const totalQuantity = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.pstock || 0), 0);
    const totalSaleRate = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.purAmt.replace(/,/g, '') || 0), 0);
            ...
            <header
                ...
                <div style={{ marginLeft: '60px', marginRight: '20px' }}>
                    <p style={{ margin: '0', fontFamily: 'Sans-serif', fontWeight: '700', fontSize: '15px', lineHeight: '1', textAlign: 'left', color: 'white' }}>
                        Reports &nbsp;&gt;&nbsp; Item Reports &nbsp;&gt;&nbsp; Item Stock Report
                    </p>
                </div>
                                    ...
                                    filteredDailysale
                                        .map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                <td style={{width:'50px'}}>{item.stock}</td>
                                                <td className='table-col-style' style={{ width: '50px' }}>{item.pstock}</td>
                                                <td className='daily-col-amount' >{item.purAmt}</td>
