...
export default function ItemMarginReport() {
    ...
    const dashboardUrl = "https://crystalsolutions.com.pk/iqbaltrader/web/ItemMarginReport.php";
    ...
    const handleCsvDownload = () => {
        // Define table column headers
        const headers = ['Date', 'Description', 'Rate', 'Qty', 'Amount', 'Margin'];
        ...
        filteredDailysale.forEach(item => {
            ...
            const row = [
                ...
                `"${item.description}"`,
                item.purcost,
                item.totalsale.toString(), // Convert to string to handle large numbers
                item.saleamount.toString(),
                item.margin.toString() // Convert to string to handle large numbers
            ];
        ...
        // Calculate totals
        const totalSaleRate = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.totalsale || 0), 0);
        const totalQuantity = filteredDailysale.reduce((acc, item) => acc + parseInt(item.saleamount || 0), 0);
    ...
    const totalopen = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.totalsale.replace(/,/g, '') || 0), 0);
    const totalopenstk = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.saleamount || 0), 0);
    const totalpurqnt = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.margin.replace(/,/g, '') || 0), 0);
            ...
            <header
                ...
                <div style={{ marginLeft: '60px', marginRight: '20px' }}>
                    <p style={{ margin: '0', fontFamily: 'Sans-serif', fontWeight: '700', fontSize: '15px', lineHeight: '1', textAlign: 'left', color: 'white' }}>
                        Reports &nbsp;&gt;&nbsp; Item Reports &nbsp;&gt;&nbsp; Item Margin Report
                    </p>
                </div>

            </header>

            <Container style={{ width: '800px', display: 'flex', justifyContent: 'center', border: '1px solid #cccccc' }} className="mt-2" >
                                    ...
                                    filteredDailysale
                                        .map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className='table-col-item-description' style={{ textAlign: 'left', cssText: 'text-align: left !important', width: '200px' }}>{item.description}</td>
                                                <td className='table-col-style' style={{ width: '70px' }}>{item.purcost}</td>
                                                <td className='table-col-style' style={{ width: '30px' }}>{item.totalsale}</td>
                                                <td className='table-col-stylet' style={{ width: '70px' }}>{item.saleamount}</td>
                                                <td className='table-col-style' style={{ width: '70px' }}>{item.margin}</td>
                                            </tr>
                                        ))
                                )}
      ...
      {Array.from({ length: 8 }).map((_, colIndex) => (
        <td key={`blank-${rowIndex}-${colIndex}`}>&nbsp;</td>
                            ...
                            <tr style={{ position: 'sticky', bottom: '0', backgroundColor: 'rgb(235, 135, 64)' }}>
                                ...
                                <th style={{ textAlign: 'right', cssText: 'text-align: right !important' }}>{formattedTotalpurqnt}</th>
