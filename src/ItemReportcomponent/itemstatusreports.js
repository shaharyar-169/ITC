...
export default function ItemStatusReport() {
    ...
    const dashboardUrl = "https://crystalsolutions.com.pk/iqbaltrader/web/ItemStatusReport.php";
        ...
        doc.autoTable({
            ...
            didDrawCell: (data) => {
                if (data.column.index === 5) { // Sale Rate column
                    doc.text(formattedTotalopen, data.cell.x + 2, data.cell.y + data.cell.height / 2,);
                } else if (data.column.index === 6) { // Quantity column
                    doc.text(formattedTotalopenstk.toString(), data.cell.x + 2, data.cell.y + data.cell.height / 2,);
                } else if (data.column.index === 7) { // Amount column
                    doc.text(formattedTotalpurqnt, data.cell.x + 2, data.cell.y + data.cell.height / 2,);
                }
            }
        });
    ...
    const totalopen = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.Opening.replace(/,/g, '') || 0), 0);
    const totalopenstk = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.OpenStk || 0), 0);
    const totalpurqnt = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.purQty.replace(/,/g, '') || 0), 0);

    const totalpurchase = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.purAmt.replace(/,/g, '') || 0), 0);
    const totalsalQty = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.salQty || 0), 0);
    const totalsalAmt = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.salAmt.replace(/,/g, '') || 0), 0);
    const totalBalance = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.Balance.replace(/,/g, '') || 0), 0);
    const totalbalanceAmt = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.balanceAmt || 0), 0);


    // Format totals with commas
    const formattedTotalopen = totalopen.toLocaleString('en-US', { maximumFractionDigits: 2 });
    const formattedTotalopenstk = totalopenstk.toLocaleString('en-US', { maximumFractionDigits: 2 });
    const formattedTotalpurqnt = totalpurqnt.toLocaleString('en-US', { maximumFractionDigits: 2 });

    const formattedtotalpurchase = totalpurchase.toLocaleString('en-US', { maximumFractionDigits: 2 });
    const formattedtotalsalQty = totalsalQty.toLocaleString('en-US', { maximumFractionDigits: 2 });
    const formattedtotalsalAmt = totalsalAmt.toLocaleString('en-US', { maximumFractionDigits: 2 });
    const formattedtotalBalance = totalBalance.toLocaleString('en-US', { maximumFractionDigits: 2 });
    const formattedtotalbalanceAmt = totalbalanceAmt.toLocaleString('en-US', { maximumFractionDigits: 2 });
            ...
            <header
                ...
                <div style={{ marginLeft: '60px', marginRight: '20px' }}>
                    <p style={{ margin: '0', fontFamily: 'Sans-serif', fontWeight: '700', fontSize: '15px', lineHeight: '1', textAlign: 'left', color: 'white' }}>
                        Reports &nbsp;&gt;&nbsp; Item Reports &nbsp;&gt;&nbsp; Item Status Report
                    </p>
                </div>

            </header>

            <Container style={{ width: '900px', display: 'flex', justifyContent: 'center', border: '1px solid #cccccc' }} className="mt-2" >
                ...
                <div className="parent ">
                        <div className="child1">
                            ...
                            {/* To Date */}
                            <div style={{ marginLeft: '120px' }} className='d-flex align-items-center'>
               ...
               <div id="lastDiv">
                            <div  className="d-flex align-items-center" style={{ marginLeft: '50px' }}>
                                <label htmlFor="searchInput" style={{ paddingRight: '21px' }}><span style={{ fontSize: '10px' }}>Search:</span>  </label>
                                <input
                                    type="text"
                                    id="searchInput"
                                    ...
                                    filteredDailysale
                                        .map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className='table-col-style'>{item.Opening}</td>
                                                <td className='table-col-style' style={{ width: '30px' }}>{item.OpenStk}</td>
                                                <td className='table-col-stylet' >{item.purQty}</td>
                                                <td className='table-col-style'>{item.purAmt}</td>
                                                <td className='table-col-style' style={{ width: '30px' }}>{item.salQty}</td>
                                                <td className='table-col-style' >{item.salAmt}</td>
                                                <td className='table-col-style' style={{ width: '30px' }}>{item.Balance}</td>
                                                <td className='daily-col-amount' >{item.balanceAmt}</td>
                                            </tr>
                                        ))
                                )}
      ...
      {Array.from({ length: 8 }).map((_, colIndex) => (
        <td key={`blank-${rowIndex}-${colIndex}`}>&nbsp;</td>
                            ...
                            <tr style={{ position: 'sticky', bottom: '0', backgroundColor: 'rgb(235, 135, 64)' }}>
                                ...
                                <th >{formattedTotalopen}</th>
                                <th>{formattedTotalopenstk}</th>
                                <th >{formattedTotalpurqnt}</th>
                                <th>{formattedtotalpurchase}</th> 
                                <th>{formattedtotalsalQty}</th> 
                                <th>{formattedtotalsalAmt}</th> 
                                <th>{formattedtotalBalance}</th> 
                                <th style={{ textAlign: 'right', cssText: 'text-align: right !important' }}>{formattedtotalbalanceAmt}</th> 
                            </tr>
