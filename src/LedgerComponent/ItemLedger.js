...
export default function ItemLedgerReport() {
  ...
  const dashboardUrl = "https://crystalsolutions.com.pk/iqbaltrader/web/ItemLedger.php";
  ...
  const totalQuantity = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.Receive.replace(/,/g, '') || 0), 0);
  const totaldebit = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.Issue.replace(/,/g, '') || 0), 0);
  const totalcredit = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.purchase || 0), 0);
  const totalbalance = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.PurchaseReturn.replace(/,/g, '') || 0), 0);
  const totalqnt = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.sale.replace(/,/g, '') || 0), 0);
  const totalSRet = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.salereturn || 0), 0);
  const totalSRet1 = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.balance || 0), 0);
  ...
  useEffect(() => {
    const apiUrl = 'https://crystalsolutions.com.pk/emart/web/CustomerList.php';
    axios.post(apiUrl)
      .then(response => {
      ...
      <header
        ...
        <div style={{ marginLeft: '60px', marginRight: '20px' }}>
          <p style={{ margin: '0', fontFamily: 'Sans-serif', fontWeight: '700', fontSize: '15px', lineHeight: '1', textAlign: 'left', color: 'white' }}>
            Reports &nbsp;&gt;&nbsp; Daily Reports &nbsp;&gt;&nbsp; Item Ledger Report
          </p>
        </div>

      </header>

      <Container style={{ height: '640px', width: '900px', display: 'flex', justifyContent: 'center', border: '1px solid #cccccc' }} className="mt-3 ">
        <div className="mt-2">
          ...
          <div className="child1" style={{ marginBottom: '5px' }}>
              ...
              {/* To Date */}
              <div style={{ marginLeft: '113px' }} className='d-flex align-items-center'>
              ...
              <MDBTable responsive scrollY striped bordered maxHeight="550px" id='dailysale-table' >
                ...
                <MDBTableBody id="dailysale-table" className='scrollable-body' >
                  ...
                  {showSpinner ? (
                    ...
                    filteredDailysale
                      .map((item, index) => (
                        <tr key={index}>
                          <td className='table-col-style' style={{ width: '80px' }}>{item.ttrndat}</td>
                          <td className='table-col-style' style={{ width: '60px' }}>{item.ttrnnum}</td>
                          <td className='table-col-item-description' style={{ textAlign: 'left', cssText: 'text-align: left !important', width: '250px' }}>{item.description}</td>
                          <td className='table-col-style' style={{ width: '50px' }}>{item.Receive}</td>
                          <td className='table-col-style' style={{ width: '50px' }}>{item.Issue}</td>
                          <td className='table-col-style' style={{ width: '50px' }}>{item.purchase}</td>
                          <td className='table-col-style' style={{ width: '50px' }}>{item.PurchaseReturn}</td>
                          <td className='supplier-col-amount' style={{ width:'50px'}}>{item.sale}</td>
                          <td className='table-col-style' style={{ width: '50px' }}>{item.salereturn}</td>
                          <td className='table-col-style' style={{ width: '100px'}}>{item.balance}</td>
                ...
                <tr style={{ position: "sticky", bottom: 0, backgroundColor: "rgb(235, 135, 64)", marginTop: '2px', }}>
                  <th className="icon-container">
                    <FontAwesomeIcon icon={faFilePdf} className="custom-icon"  title="Download PDF" />
                    <FontAwesomeIcon icon={faFileCsv} className="custom-icon" title="Download CSV" />
                  </th>
