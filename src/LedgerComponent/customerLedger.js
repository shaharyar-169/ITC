...
export default function CustomerLedgerReport() {
  ...
  const handleSaleKeypress = (event, inputId) => {
    if (event.key === 'Enter') {
      setTimeout(() => { // Ensure this code runs after the default behavior
        const selectedOption = saleSelectRef.current && saleSelectRef.current.state && saleSelectRef.current.state.selectValue;
        if (selectedOption) {
          setSaleType(selectedOption.value); // Set the selected value
        }
        const nextInput = document.getElementById(inputId);
        if (nextInput) {
  ...
  const dashboardUrl = "https://crystalsolutions.com.pk/iqbaltrader/web/CustomerLedger.php";
  ...
  const totalbalance = filteredDailysale.reduce((acc, item) => {
    const balance = item.balance || '0'; // If item.balance is null, use '0' as default
    return acc + parseFloat(balance.replace(/,/g, '') || 0);
}, 0);
  ...
  useEffect(() => {
    const apiUrl = 'https://crystalsolutions.com.pk/iqbaltrader/web/CustomerList.php';
    axios.post(apiUrl)
      .then(response => {
      ...
      <header
        ...
        <div style={{ marginLeft: '60px', marginRight: '20px' }}>
          <p style={{ margin: '0', fontFamily: 'Sans-serif', fontWeight: '700', fontSize: '15px', lineHeight: '1', textAlign: 'left', color: 'white' }}>
            Reports &nbsp;&gt;&nbsp; Daily Reports &nbsp;&gt;&nbsp; Supplier Ledger Report
          </p>
        </div>
        ...
        <div className="mt-2">
          <div className="parent ">
            <div className="child1" style={{ marginBottom: '5px' }}>
              ...
              {/* To Date */}
              <div style={{ marginLeft: '224px' }} className='d-flex align-items-center'>
