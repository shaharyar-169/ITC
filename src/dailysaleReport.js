...
export default function DailySaleReport() {
  ...
  const dashboardUrl = "https://crystalsolutions.com.pk/iqbaltrader/DailySaleReport.php";
  ...
  const handleSaleTypeChange = (event) => {
    ...
    const filteredData = filterData(saleType, transectionType, previousFilteredData);
    setFilteredDailysale(filteredData);
    setpreviousFilteredData(filteredData);
    // console.log('filtered dailysale data', response.data);
      ...
      <header
        ...
        <div style={{ marginLeft: '60px', marginRight: '20px' }}>
          <p style={{ margin: '0', fontFamily: 'Sans-serif', fontWeight: '700', fontSize: '15px', lineHeight: '1', textAlign: 'left', color: 'white' }}>
            Reports &nbsp;&gt;&nbsp; Daily Reports &nbsp;&gt;&nbsp; Daily Sales Report
          </p>
        </div>

      </header>

      <Container style={{ width:'1000px', display: 'flex', justifyContent: 'center', border: '1px solid #cccccc' }} className="mt-2" >
        <div className="pt-2 pb-2" style={{width:'950px'}}>
        <div className="parent ">
          <div className="child1">
              ...
              {/* To Date */}
              <div style={{ marginLeft: '97px' }} className='d-flex align-items-center'>
              ...
              <MDBTableBody id="dailysale-table-body">
                  ...
                  filteredDailysale
                    .map((item, index) => (
                      <tr key={index}>
                        ...
                        <td style={{ width: '100px' }}>{item.tsalrat}</td>
                ...
                {Array.from({ length: Math.max(24 - filteredDailysale.length, 0) }).map((_, rowIndex) => (
                  <tr key={`empty-row-${rowIndex}`}>
                    ...
                    <td className={`table-col-style ${filteredDailysale.length === 0 ? 'empty-style' : ''}`} style={{ width: '150px' }}></td>
      ...
      {Array.from({ length: 8 }).map((_, colIndex) => (
        <td key={`blank-${rowIndex}-${colIndex}`}>&nbsp;</td>
              ...
              <tr style={{ position: 'sticky', bottom: '0', backgroundColor: 'rgb(235, 135, 64)' }}>
                ...
                <th colSpan={4} style={{ textAlign: 'left !important' }}>TOTAL</th>
