...
export default function CashBankBalanceRepoert() {
  ...
  const dashboardUrl = "https://crystalsolutions.com.pk/emart/web/CashAndBankBalance.php";
  ...
  const handleCsvDownload = () => {
    ...
    const totalQuantity = filteredDailysale.reduce((acc, item) => acc + parseInt(item.debit || 0), 0);
    const totalAmount = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.credit || 0), 0);
  ...
  const totalQuantity = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.debit || 0), 0);
  const totalSaleRate = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.credit.replace(/,/g, '') || 0), 0);
  const totalclosing = filteredDailysale.reduce((acc, item) => acc + parseFloat(item.closing.replace(/,/g, '') || 0), 0);
      ...
      <header
        ...
        <div style={{ marginLeft: '60px', marginRight: '20px' }}>
          <p style={{ margin: '0', fontFamily: 'Sans-serif', fontWeight: '700', fontSize: '15px', lineHeight: '1', textAlign: 'left', color: 'white' }}>
            Reports &nbsp;&gt;&nbsp; Daily Reports &nbsp;&gt;&nbsp; Cash & Cash Bank Balance 
          </p>
        </div>

      </header>

      <Container style={{ height:'640px', width: '900px', display: 'flex', justifyContent: 'center', border: '1px solid #cccccc' }} className="mt-2" >
        <div className="mt-2">
          <div className="parent ">
            <div className="child1" style={{marginBottom:'5px'}}>
              ...
              {/* To Date */}
              <div style={{ marginLeft: '80px' }} className='d-flex align-items-center'>
              ...
              <div style={{ marginLeft: '50px' }}>
                <label htmlFor="searchInput" style={{ paddingRight: '10px' }}><span style={{ fontSize: '12px' }}>Search</span>  </label>
                <input
                  type="text"
                  id="searchInput"
                  placeholder="Item description"
                  value={searchQuery}
                  style={{ width: '200px', height: '32px', fontSize: '12px', border: '1px solid black', outline: 'none', paddingLeft: '10px' }}
            ...
            {/* <div className="child2" >
              <div className="d-flex">
                <div >
                  <label htmlFor="sale" style={{ marginTop: '10px' }}><span style={{ fontSize: '12px' }}>Sale:</span> </label>
                  <select
                    class="select"
                    ref={saleSelectRef}
                    ...
                    id="selectedsale"
                    name="sale"
                    style={{ width: '135px', fontSize: '12px', height: '32px', marginLeft: '51px', textAlign: 'center', background: 'white', border: '1px solid black' }}
                    value={saleType}
                ...
                <div style={{ marginLeft: '80px' }}>
                  <label htmlFor="type" style={{ marginTop: '10px' }} ><span style={{ fontSize: '12px' }}>Type:</span> </label>
              ...
              <div style={{ marginLeft: '50px' }}>
                ...
                <input
                  ...
                  placeholder="Item description"
                  value={searchQuery}
                  style={{ width: '200px', height: '32px', fontSize: '12px', border: '1px solid black', outline: 'none', marginTop: '7px', paddingLeft: '10px' }}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
          ...
          <Row className="gap-3" >
            ...
            <MDBTable responsive striped scrollY maxHeight="560px" id='dailysale-table' className="MDB-table" >
              ...
              <MDBTableBody id="dailysale-table-body">
                  ...
                  filteredDailysale
                    .map((item, index) => (
                      <tr key={index}>
                        ...
                        <td style={{ width: '280px' , textAlign:'left'}}>{item.description}</td>
                        <td style={{ width: '100px' , textAlign:'right'}}>{item.opening}</td>
                        <td style={{ width: '100px' , textAlign:'right'}}>{item.debit}</td>
                        <td style={{width: '100px' , textAlign:'right'}}>{item.credit}</td>
                        <td className='daily-salerat'>{item.closing}</td>
                ...
                {Array.from({ length: Math.max(19 - filteredDailysale.length, 0) }).map((_, rowIndex) => (
                  <tr key={`empty-row-${rowIndex}`}>
                    <td className={`table-col-style ${filteredDailysale.length === 0 ? 'empty-style' : ''}`} style={{ width: '70px' }}>&nbsp;</td>
                    <td className={`table-col-style ${filteredDailysale.length === 0 ? 'empty-style' : ''}`} style={{ width: '280px' }}></td>
                    <td className={`table-col-style ${filteredDailysale.length === 0 ? 'empty-style' : ''}`} style={{ width: '100px' }}></td>
                    <td className={`table-col-style ${filteredDailysale.length === 0 ? 'empty-style' : ''}`} style={{ width: '100px' }}></td>
                    <td className={`table-col-item-description ${filteredDailysale.length === 0 ? 'empty-style' : ''}`} style={{ width: '100px' }}></td>
                    <td className={`table-col-style ${filteredDailysale.length === 0 ? 'empty-style' : ''}`} style={{ width: '100px' }}></td>
                      </tr>
                ))}
      ...
      {Array.from({ length: 8 }).map((_, colIndex) => (
        <td key={`blank-${rowIndex}-${colIndex}`}>&nbsp;</td>
              ...
              <tr style={{ position: 'sticky', bottom: '0', backgroundColor: 'rgb(235, 135, 64)' }}>
                ...
                <th style={{ textAlign: 'right', cssText: 'text-align: right !important' }}>{totalQuantity}</th>
                <th style={{ textAlign: 'right', cssText: 'text-align: right !important' }}>{formattedTotalAmount}</th>
                <th style={{textAlign:'right', cssText:'text-align: right !important'}}>{totalclosing}</th>
              </tr>
